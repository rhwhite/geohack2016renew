...
def read_params(txt):
    ...
    try:
        ...
        sys.exit('Problem reading parameter file: %s' % txt)
...
def main(params):
    ...
    '''if 'nodata' in locals():
        ...
        nodata = None'''
    if 'pct_train_zone' in locals():
        pct_train_zone = float(pct_train_zone)
    else:
        raise NameError('pct_train_zone not specified')
    if 'pct_train_sample' in locals():
        pct_train_sample = float(pct_train_sample)
    else:
        pct_train_sample = None
    if 'false_val' in locals():
        false_val = int(false_val)
    else:
        print 'WARNING: no false_val given in param file. Using 0 as default'
        false_val = 0
    if 'false_inflation' in locals():
        false_inflation = float(false_inflation)
    else:
        false_inflation = None
    ...
    df_train, df_test, raster_res = forest.get_samples_from_zones(raster_path, col_name, data_band, n_samples, false_val, pct_train_sample, pct_train_zone, nodata, false_inflation)
    ...
    stamp = '{0}_{1}_{2}_{3}m'.format(len(df_train), date_str, time_str, int(round(raster_res,0)))
    ...
    if 'test_n_trees_params' in locals():
        if not 'var_txt' in locals():
            print 'Cannot test number of trees becuase no predictors were sampled. Try specifying a var_txt path.'
